...
<p align="center">
  <a href="https://dodouuuu.github.io/vcard-avatar-generator/">在线演示</a>
</p>
...
- **头像风格** — 基于 [DiceBear](https://www.dicebear.com/) v10，支持多种头像风格切换和自定义配置
...
- **头像引擎**: [DiceBear](https://www.dicebear.com/) v10
...
> 如果你习惯使用 Excel，可以下载页面提供的 [XLSX 模板](public/asset/vcard-template.xlsx)，填写联系人信息后再导入。
...
├── config/
│   ├── dicebear.ts             # DiceBear 默认配置
│   └── spotcard.ts             # 侧边栏展示位配置
├── components/
│   ├── Header.svelte           # 顶栏（标题 + GitHub 链接）
│   ├── Footer.svelte           # 底栏（版权信息 + 构建时间）
│   ├── FileUploader.svelte     # 文件上传组件（拖拽/选择/解析）
│   └── avatar/
│       ├── AvatarImg.svelte    # 头像渲染组件
│       ├── AvatarOpt.svelte    # 头像选项组件
│       └── AvatarPanel.svelte  # 头像配置面板（风格/颜色/组件）
├── pages/
│   ├── Upload.svelte           # 文件上传页（三栏布局）
